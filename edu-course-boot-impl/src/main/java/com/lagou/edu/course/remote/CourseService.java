...
import com.lagou.edu.course.client.dto.*;
...
public class CourseService {
    ...
    */
    ...
    private void setCourseLastPlayTimeAndLessonName(Integer userId, Integer courseId, CourseDTO courseDTO) {
       ...
       courseDTO.setCompareTime(LocalDateUtil.asDate(coursePlayHistory.getUpdateTime()));
    ...
    private List<SectionDTO> getSectionDTOS(QueryWrapper lessonQueryWrapper, Course course, List<Section> sections) {
        ...
        for (Section section : sections) {
            ...
            CoverUtil.cover(section,sectionDTO);
            ...
            for (Lesson lesson : lessons) {
                ...
                CoverUtil.cover(lesson,lessonDTO);
    ...
    public boolean saveCourse(@RequestBody CourseDTO courseDTO) {
        ...
        CoverUtil.cover(courseDTO,course);
        ...
        CoverUtil.cover(teacherDTO,teacher);
    ...
    public PageResultDTO<CourseDTO> getQueryCourses(@RequestBody CourseQueryParam courseQueryParam){
        PageHelper.startPage(courseQueryParam.getCurrentPage(),courseQueryParam.getPageSize());
        QueryWrapper<Course> courseQueryWrapper = new QueryWrapper<>();
        if(StringUtils.isNotEmpty(courseQueryParam.getCourseName())){
            courseQueryWrapper.like("courseName",courseQueryParam.getCourseName());
        }
        if(courseQueryParam.getStatus() != null){
            courseQueryWrapper.eq("status",courseQueryParam.getStatus());
        }
        List<Course> rs = courseMapper.selectList(courseQueryWrapper);
        PageInfo<Course> coursePageInfo = new PageInfo<>(rs);
        List<CourseDTO> courseDTOS = CoverUtil.coverList(coursePageInfo.getList(), CourseDTO.class);
        PageResultDTO<CourseDTO> courseDTOPageResultDTO = new PageResultDTO<>();
        courseDTOPageResultDTO.setRecords(courseDTOS);
        courseDTOPageResultDTO.setPages(coursePageInfo.getPages());
        courseDTOPageResultDTO.setCurrent(coursePageInfo.getPageNum());
        courseDTOPageResultDTO.setSize(coursePageInfo.getSize());
        courseDTOPageResultDTO.setTotal(coursePageInfo.getTotal());
        return courseDTOPageResultDTO;
    }

    public CourseDTO getCourseById(Integer courseId, Integer userId){
        Course course = courseMapper.selectById(courseId);
        CourseDTO courseDTO = CoverUtil.cover(course, CourseDTO.class);
        Teacher teacher = Optional.ofNullable(teacherService.getBaseMapper().selectOne(new QueryWrapper<Teacher>().eq("course_id", courseId))).orElse(new Teacher());
        courseDTO.setTeacherDTO(CoverUtil.cover(teacher,TeacherDTO.class));
        return courseDTO;
    }
